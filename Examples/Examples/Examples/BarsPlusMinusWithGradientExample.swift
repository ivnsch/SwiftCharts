...
class BarsPlusMinusWithGradientExample: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let bars: [ChartPointsBar] = Array(enumerate(vals)).map {index, tuple in
            ...
            return ChartPointsBar(constant: ChartAxisValueFloat(CGFloat(index)), axisValue1: zero, axisValue2: ChartAxisValueFloat(tuple.val), bgColor: color)
        }
        ...
        let barsLayer = ChartBarsLayer(xAxis: xAxis, yAxis: yAxis, innerFrame: innerFrame, bars: bars, horizontal: true, barWidth: Env.iPad ? 40 : 16)
        
        var settings = ChartGuideLinesLayerSettings(linesColor: UIColor.blackColor(), linesWidth: ExamplesDefaults.guidelinesWidth, axis: .X)
