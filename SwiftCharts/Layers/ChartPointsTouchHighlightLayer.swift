...
public class ChartPointsTouchHighlightLayer<T: ChartPoint, U: UIView>: ChartPointsViewsLayer<T, U> {
    ...
    public init(xAxis: ChartAxis, yAxis: ChartAxis, chartPoints: [T], gestureRecognizer: UIPanGestureRecognizer? = nil, modelFilter: ChartPointLayerModelForScreenLocFilter, viewGenerator: ChartPointViewGenerator) {
    ...
    var highlightedModel: ChartPointLayerModel<T>? {
        didSet {
            if highlightedModel?.index != oldValue?.index, let view = view, chart = chart {
                ...
                if let model = highlightedModel, pointView = viewGenerator(chartPointModel: model, layer: self, chart: chart, isTransform: false) {
