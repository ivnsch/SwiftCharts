...
public class LineChart: Chart {
    ...
    public init(frame: CGRect, chartConfig: ChartConfigXY, xTitle: String, yTitle: String, lines: [ChartLine]) {
        ...
        let xValues = Array(stride(from: chartConfig.xAxisConfig.from, through: chartConfig.xAxisConfig.to, by: chartConfig.xAxisConfig.by)).map{ChartAxisValueDouble($0)}
        let yValues = Array(stride(from: chartConfig.yAxisConfig.from, through: chartConfig.yAxisConfig.to, by: chartConfig.yAxisConfig.by)).map{ChartAxisValueDouble($0)}
